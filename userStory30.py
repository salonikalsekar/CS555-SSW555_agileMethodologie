...
def livingPeopleMarried(listIndi, listFam):
    marriedPeopleList = []
    for i in listFam:
        if(getDeathDate(listIndi, i[1]) == None):
            if(i[1] not in marriedPeopleList):
                marriedPeopleList.append(i[1])
        if(getDeathDate(listIndi, i[2]) == None):
            if(i[2] not in marriedPeopleList):
                marriedPeopleList.append(i[2])
    print ("User story 30: Individuals who are married and alive", marriedPeopleList)
    for i in marriedPeopleList:
        print("ID:" + i + " with name " + getIndiName(listIndi, i) + " is married with family " + str(getSpouseFamily(listIndi, i)))
...
def main(fileName):
    ...
    listIndi.sort()
    listFam.sort()
    livingPeopleMarried(listIndi, listFam)
