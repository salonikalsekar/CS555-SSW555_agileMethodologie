...
def maleLastNames(list_indi, list_fam):
    ...
    for j in list_fam:
        ...
        if(len(set(male_names)) != 1):
            list1.append(j[0])
            print('US16: Family ' + j[0] + ' has one or more male members with different last name(s).')
    if(len(list1)==0):
        print("US16: Males in all families have the same last name.")
        print()
    else:
        print(" US16: Families which have one or more male members with different last name(s): ", end = '')
        print(list1)
        print()
